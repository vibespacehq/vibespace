	...
	tests := []struct {
	...
	}{
		{
			...
			components: ClusterComponents{
				...
				BuildKit: ComponentStatus{Installed: true, Healthy: true},
			},
			expected: true,
		},
		{
		...
		},
		{
			...
			components: ClusterComponents{
				...
				BuildKit: ComponentStatus{Installed: false, Healthy: false},
			},
			expected: false,
		},
	}
...
func TestGetMissingComponents(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: []string{"knative", "traefik", "registry", "buildkit"},
		},
	}
