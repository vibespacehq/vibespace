...
// AgentSessions tracks coding agent sessions for one agent within a multi-session
...
// AgentSessionManager manages coding agent CLI sessions across all agents and multi-sessions
...
// Sessions are persisted to ~/.vibespace/agent_sessions.json
func NewAgentSessionManager() *AgentSessionManager {
	homeDir, _ := os.UserHomeDir()
	filePath := filepath.Join(homeDir, ".vibespace", "agent_sessions.json")
...
func (m *AgentSessionManager) GetOrCreateSession(multiSessionID, agentAddr string) string {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AgentSessionManager) GetSession(multiSessionID, agentAddr string) string {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AgentSessionManager) NewSession(multiSessionID, agentAddr string) string {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AgentSessionManager) ResumeSession(multiSessionID, agentAddr, sessionID string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *AgentSessionManager) ListSessions(multiSessionID, agentAddr string) []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AgentSessionManager) GetCurrentSessionID(multiSessionID, agentAddr string) string {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *AgentSessionManager) save() {
